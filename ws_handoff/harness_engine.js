const React={createElement:()=>{}};const localStorage={getItem:()=>null,setItem:()=>{}};const btoa=x=>x;const atob=x=>x;
...
const DEFS = {
  ...
  thp_t09_alice: {
    ...
    text: '【永】 応援對前方角色全部 +（該角色等級×500）力量。\n【自】 自方觸發為「門」的卡觸發時，選擇 1 隻我方《幻想郷》角色，當回合 +2000 力量。'
  },
  thp_021_mystia: {
...
// アリス門觸發加成：觸發方場上有 TRIGGER_GATE_BUFF_GENSO_2000 時，選1隻幻想郷角色 +2000
function applyAliceGateBuff(s, pIdx) {
  const P = s.players[pIdx];
  ...
  if (isHuman(s, pIdx)) {
    s.pending = { type: 'CHARSEL_BUFF', pIdx, amount: 2000, cand: cands, source: 'アリス·門觸發', filterGensokyo: true };
  } else {
    const best = cands.reduce((a, b) => calcPower(s, pIdx, a) >= calcPower(s, pIdx, b) ? a : b);
    const bc = P.stage[best];
    bc.autoBuff = bc.autoBuff || { power: 0, soul: 0 };
    bc.autoBuff.power += 2000;
    s.log.push(`🟡 アリス門觸發：${bc.def.name} 當回合 +2000。`);
  }
  return s;
}
function doGate(s, pIdx, cxCard) {
  ...
  // NPC：立即處理 Alice 門觸發加成
  s = applyAliceGateBuff(s, pIdx);
  return s;
}
    ...
    case 'GATE':
      {
        ...
        // 閘結算後：檢查アリス門觸發加成（若有則 pending，否則自動續接 counter）
        s = applyAliceGateBuff(s, p.pIdx);
        break;
      }
...
module.exports={checkZeroPowerDestroy,gameReducerInner,attackAfterConfirm,declareAttack};